  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">Edit base tuition fees per class and optional addon pricing</p>
      </div>

      <form onSubmit={onSubmit} className="space-y-8 w-full bg-white/80 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {Object.keys(editedFees.classes).map((cls) => (
              <div key={cls} className="space-y-1">
                <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">{cls}</label>
                <input
                  type="number"
                  value={editedFees.classes[cls]}
                  onChange={(e) => handleClassFeeChange(cls, e.target.value)}
                  className="w-full px-3 py-1.5 bg-slate-950/50 border border-slate-200 dark:border-slate-800 focus:border-amber-500 rounded-lg text-white outline-none text-xs font-mono"
                />
                <p className="text-[9px] text-slate-500 dark:text-slate-500 font-mono">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Transport Service Fee</label>
              <input
                type="number"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">School Dress / Uniform Set</label>
              <input
                type="number"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Books & Stationery Kit</label>
              <input
                type="number"
